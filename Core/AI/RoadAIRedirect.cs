...
{
    public class RoadAIRedirect : RoadBaseAI
    {
        private void CreateZoneBlocks(ushort segment, ref NetSegment data)
        {
            try
            {
                ...
                var halfWidth = Mathf.Max(roadUnitSize, this.m_info.m_halfWidth);
                ...
                else
                {
                    ...
                    var rows1 = (nbUnit <= roadUnitSize) ? nbUnit : (nbUnit + 1 >> 1);
                    var rows2 = (nbUnit <= roadUnitSize) ? 0 : (nbUnit >> 1);
